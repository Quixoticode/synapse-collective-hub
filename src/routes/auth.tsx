...
import { XsynaAccountCard, type XsynaAccountCardData } from "@/components/XsynaAccountCard";

export const Route = createFileRoute("/auth")({
  ssr: false,
  head: () => ({
    meta: [
      { title: "Anmelden – xSyna Central" },
      { name: "description", content: "Sichere Anmeldung zum xSyna-Kollektiv." },
    ],
  }),
  component: AuthPage,
});
...
function AuthPage() {
  ...
  const [verified, setVerified] = useState<XsynaAccountCardData | null>(null);
  ...
  async function handle(rawPik: string) {
    ...
    try {
      if (stage === "login") {
        ...
        if (res.valid) { setVerified(res.card as XsynaAccountCardData); setVerifyState("ok"); }
        else { setVerified(null); setVerifyState("fail"); setError("Account nicht gültig."); }
      }
    } catch (e) {
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-5 sm:mb-7">
          ...
          <p className="mt-2 text-xs sm:text-sm text-muted-foreground">
            Login &amp; Verifikation über das xSyna Account-Modul.
          </p>
        </div>
        ...
        <div className="grid grid-cols-5 gap-1.5 mb-4">
          ...
          <StageBtn active={stage === "login"} onClick={() => { setStage("login"); setVerified(null); setError(null); }} icon={<LogIn className="h-4 w-4" />} label="Account" />
        ...
        {stage === "support" ? (
          ...
          <div className="syn-card p-4 sm:p-6 space-y-4 syn-gradient-border" style={{ borderColor: accent }}>
            ...
            {stage === "verify" && verifyState === "fail" && !error && (
              <div className="text-xs mono p-2 rounded-lg bg-destructive/10 border border-destructive/30 text-destructive">Account ungültig.</div>
            )}

            <div className="flex items-start gap-2 text-[11px] text-muted-foreground">
              <ShieldCheck className="h-3 w-3 mt-0.5 shrink-0" style={{ color: "var(--neural-mint)" }} />
              <span>{stage === "login" ? "PIK wird verschlüsselt übertragen und nur serverseitig geprüft." : "Verifikation liefert nur die öffentlichen Account-Infos – keine Sitzung."}</span>
            </div>
          </div>
        )}
  ...
  function SupportSection() {
    ...
    return (
      <div className="syn-card syn-gradient-border p-5 space-y-4" style={{ borderColor: "var(--neural-magenta)" }}>
        ...
        <p className="text-[11px] text-muted-foreground">Support-Accounts benötigen keinen xSyna Account – nur Name + Code für dein Ticket.</p>
      </div>
    );
  }
  ...
  function QuickLoginSection({ onDone }: { onDone: (s: {
    ...
    return (
      <div className="syn-card p-5 space-y-3 syn-gradient-border" style={{ borderColor: "var(--synapse)" }}>
        ...
        <input className="syn-input" placeholder="Account-ID (z. B. K12345678)" value={slid} onChange={(e) => setSlid(e.target.value.toUpperCase())} />
        <input className="syn-input mono tracking-widest text-center text-lg" placeholder="6-stelliger Code" maxLength={6} value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, ""))} />
