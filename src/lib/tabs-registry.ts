...
// Sidebar, BottomNav, AppsLauncher, Permissions and Tab-Prefs all read from this list.
import {
  LayoutGrid, Contact, MessageSquare, Mail, KeyRound, FileText, BookOpen,
  Users, Plug, Calendar, LifeBuoy, Wallet, Newspaper, ShieldCheck, Settings, type LucideIcon,
} from "lucide-react";

export type TabKey =
  | "apps" | "contacts" | "chat" | "mail" | "vault" | "workspace" | "basics"
  | "calendar" | "support" | "finances" | "news" | "collective"
  | "permissions" | "settings.tabs" | "settings.integrations";
...
export type TabDef = {
  ...
  desc: string;
  icon: LucideIcon;
  accent: string;          // gradient for app-tile
...
export const TABS: TabDef[] = [
  ...
  { key: "mail",      to: "/mail",       label: "SynMail",   desc: "E-Mail-Konten",     icon: Mail,         accent: "from-violet-500/30 to-cyan-500/20", category: "core" },
...
export function visibleTabs(ctx: TabsCtx, rule: TabRule = {}) {
  return TABS.filter((t) => {
    ...
    // explicit permission (admin override) wins
    ...
    const sa = rule.prefs?.[a.key]?.sort_order ?? 0;
    const sb = rule.prefs?.[b.key]?.sort_order ?? 0;
    return sa - sb;
  });
}
